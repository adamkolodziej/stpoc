...
<c:choose>
	...
	<div class="productImageGallery gallery_left">
	...
	<div class="productImagePrimary gallery_left" id="primary_image">
	...
	<c:when test="${showGallery and galleryPosition eq 'right'}">
		<div class="productImagePrimary gallery_right" id="primary_image">
			<c:if test="${fn:contains(product.url, '?sku=')}">
				<c:url value="${fn:substringBefore(product.url, '?sku=')}/zoomImages" var="productZoomImagesUrl"/>
			</c:if>
			<c:if test="${not fn:contains(product.url, '?sku=')}">
		...
		<div class="productImageGallery gallery_right">
		<ul class="jcarousel-skin">
			<c:forEach items="${galleryImages}" var="container" varStatus="varStatus">
				<li>
					<span class="thumb ${(varStatus.index==0)? "active":""}">
	...
	<c:otherwise>
		<div class="productImagePrimary no_gallery" id="primary_image">
		<c:if test="${fn:contains(product.url, '?sku=')}">
			<c:url value="${fn:substringBefore(product.url, '?sku=')}/zoomImages" var="productZoomImagesUrl"/>
		</c:if>
		<c:if test="${not fn:contains(product.url, '?sku=')}">
