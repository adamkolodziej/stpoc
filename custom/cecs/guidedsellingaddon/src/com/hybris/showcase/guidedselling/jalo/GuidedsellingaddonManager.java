...
import com.hybris.showcase.guidedselling.constants.GuidedsellingaddonConstants;



/**
 * This is the extension manager of the Guidedsellingaddon extension.
 */
public class GuidedsellingaddonManager extends GeneratedGuidedsellingaddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(GuidedsellingaddonManager.class.getName());
	 ...
	 */
	public static GuidedsellingaddonManager getInstance()
	{
		return (GuidedsellingaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				GuidedsellingaddonConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public GuidedsellingaddonManager() // NOPMD 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of GuidedsellingaddonManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of GuidedsellingaddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of GuidedsellingaddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
