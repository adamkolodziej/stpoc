...
<c:if test="${fn:length(currencies) > 1}">
	...
	<form:form action="${setCurrencyActionUrl}" method="post" id="currency_form_${component.uid}" class="currency-form">
		<div class="form-group">
			<label class="control-label skip visible-print-block" for="currency_selector_${component.uid}"><spring:theme code="text.currency"/></label>
            <ycommerce:testId code="header_currency_select">
                <select name="code" id="currency_selector_${component.uid}" class="currency-selector form-control input-sm">
