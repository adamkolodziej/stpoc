...
public class BaseProductCodeProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable
{
	...
	private List<FieldValue> createFieldValue(final ProductModel product, final IndexedProperty indexedProperty)
	{
		...
		{	
			final ProductModel resolvedProduct = getSolrRollupService().getRollupProduct(product, (product instanceof VariantProductModel) && ((VariantProductModel)product).getBaseProduct() != null ? ((VariantProductModel)product).getBaseProduct() : product);
			fieldValues.add(new FieldValue(fieldName, resolvedProduct.getCode()));	
		}
		return fieldValues;
	}	
