 ...
 * 
 */
package com.hybris.productsets.cockpit.components.listview.impl;
...
public class ProductSetSynchronisationAction extends AbstractSynchronizationAction
{
	private static final Logger LOG = Logger.getLogger(ProductSetSynchronisationAction.class);
	...
	{
		final SyncContext syncCtx = getSynchronizationService().getSyncContext(context.getItem());

		context.getMap().put(SYNC_JOBS_KEY, syncCtx.getSyncJobs());
		context.getMap().put(SOURCE_CATALOG_VERSION_KEY, getSynchronizationService().getCatalogVersionForItem(context.getItem()));
			...
			private final EventListener listener = new EventListener()
			{
				public void onEvent(final Event event) throws Exception //NOPMD: ZK Specific
				{
					try
					{
						...
						{
							getSynchronizationService().performSynchronization(Collections.singletonList(context.getItem()), null, null,
									null);
						}
					}
					...
					finally
					{
						...
						UISessionUtils.getCurrentSession().sendGlobalEvent(
								new ItemChangedEvent(ProductSetSynchronisationAction.this, context.getItem(), null));
