...
<c:if test="${fn:length(languages) > 1}">
	...
	<form:form action="${setLanguageActionUrl}" method="post" id="lang_form_${component.uid}" class="lang-form">
		...
		<div class="form-group">
            <label class="control-label skip visible-print-block" for="lang_selector_${component.uid}">${languageText}</label>
            <ycommerce:testId code="header_language_select">
                <select name="code" id="lang_selector_${component.uid}" class="lang-selector form-control input-sm">
