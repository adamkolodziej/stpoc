...
<div class="orderList row">
    <div class="col-xs-12">
        <c:choose>
            <c:when test="${consignment.deliveryPointOfService ne null}">
                <div class="headline text-warning"><spring:theme code="text.account.order.title.storePickUpItems" /></div>
            </c:when>
            <c:otherwise>
                <div class="headline text-warning"><spring:theme code="text.account.order.title.deliveryItems" /></div>
            </c:otherwise>
        </c:choose>
    </div>
    ...
    <div class="col-xs-12">
        <div class="table-responsive">
            <table class="orderListTable table table-bordered table-striped table-condensed">
                <thead>
                    <tr>
                    ...
                    <c:forEach items="${consignment.entries}" var="entry">
                        ...
                        <tr class="item">
                            ...
                            <td headers="header6" class="total">
                                <ycommerce:testId code="orderDetails_productTotalPrice_label">${entry.quantity * entry.orderEntry.totalPrice.value}</ycommerce:testId>
                            </td>
                        </tr>
                    </c:forEach>
                </tbody>
