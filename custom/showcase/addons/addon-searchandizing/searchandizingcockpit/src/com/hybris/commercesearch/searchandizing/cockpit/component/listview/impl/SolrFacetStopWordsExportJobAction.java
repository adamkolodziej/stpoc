 ...
 * 
 */
public class SolrFacetStopWordsExportJobAction extends AbstractStopWordsExportJobAction
{
		...
		return new EventListener()
		{
				...
				public void onEvent(final Event event)
				{
					...
					try
					{
						...
						if (Current_Batch_Status == ACTIVE_CRON_JOBS)
						{
							...
							performStopWordsExportJob(context);
						}
			...
			public void onEvent(final Event event) throws Exception
			{
				...
				if (!(RUNNING_CRON_JOBS == ((Integer) status).intValue()))
				{
					...
					event.getTarget().removeEventListener("onStopWordsExport", this.listener);
					event.getTarget().addEventListener("onStopWordsExport", this.listener);
					...
					Events.echoEvent("onStopWordsExport", event.getTarget(), null);
				}
			}
		};
	}
		...
		if (checkTypeRestriction(context))
		{
			...
			//Running Batch Jobs
				case RUNNING_CRON_JOBS:
					tooltip = Labels.getLabel("cockpit.job.action.stopStopWords");
					break;
				//Active Batches--No Running Jobs			
				case ACTIVE_CRON_JOBS:
					tooltip = Labels.getLabel("cockpit.job.action.startStopWords");
					break;
				//No Batches Defined
