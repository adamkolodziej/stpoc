...
<c:set var="themeTicketMsgKey" value="text.account.ticketing.page"/>
...
<div class="paginationBar row ${(top)?"top":"bottom hidden-xs"}">
    <div class="col-xs-12 col-sm-6">
        <ycommerce:testId code="searchResults_productsFound_label">
            <div class="totalResults label label-primary"><spring:theme code="${themeMsgKey}.totalResults" arguments="${searchPageData.pagination.totalNumberOfResults}"/></div>
        </ycommerce:testId>
    </div>
    <div class="col-xs-12 col-sm-6">
        <c:if test="${not empty searchPageData.sorts}">
            <form id="sort_form${top ? '1' : '2'}" name="sort_form${top ? '1' : '2'}" method="get" action="#" class="sortForm form-inline">
                <label for="sortOptions${top ? '1' : '2'}"><spring:theme code="${themeMsgKey}.sortTitle"/></label>
                <select id="sortOptions${top ? '1' : '2'}" name="sort" class="sortOptions form-control input-sm ">
