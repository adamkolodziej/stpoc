...
public class SolrFacetSearchFullIndexJobAction extends AbstractFacetIndexJobListAction
{
	...
	protected static final String ICON_ACTIVE_JOBS = "cockpit/images/fullUpdate_play.png";
		...
		return new EventListener()
		{
			...
			public void onEvent(final Event event) throws Exception
			{
				...
				if (!(RUNNING_CRON_JOBS == ((Integer) status).intValue()))
				{
					...
					event.getTarget().removeEventListener("onFullIndexUpdate", this.listener);
					event.getTarget().addEventListener("onFullIndexUpdate", this.listener);
					...
					Events.echoEvent("onFullIndexUpdate", event.getTarget(), null);
				}
			}
		};
	}
		...
		if (checkTypeRestriction(context))
		{
			...
			//Running Batch Jobs
				case RUNNING_CRON_JOBS:
					tooltip = Labels.getLabel("cockpit.job.action.stopFull");
					break;
				//Active Batches--No Running Jobs			
				case ACTIVE_CRON_JOBS:
					tooltip = Labels.getLabel("cockpit.job.action.startFull");
					break;
				//No Batches Defined
