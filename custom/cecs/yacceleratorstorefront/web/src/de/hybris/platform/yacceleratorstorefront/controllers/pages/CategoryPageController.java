...
public class CategoryPageController extends AbstractCategoryPageController {
	...
	public String category(@PathVariable("categoryCode") final String categoryCode,
		...
		final String view = performSearchAndGetResultsPage(categoryCode, searchQuery, page, showMode, sortCode, model, request, response);

		boolean redirect = view != null && view.startsWith(REDIRECT_PREFIX);
		// itemtype could be exchanged with something else
		boolean useDefault = searchQuery != null && searchQuery.contains("itemtype");

		if (!redirect && !useDefault && StringUtils.isNotBlank(categoryCode))
		{
			final CategoryModel newCategory = getCommerceCategoryService().getCategoryForCode(categoryCode);
			CategoryPageModel categoryPage = getCategoryPage(newCategory);
