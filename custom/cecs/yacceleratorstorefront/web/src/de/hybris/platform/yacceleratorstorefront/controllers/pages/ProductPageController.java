...
public class ProductPageController extends AbstractPageController
{
	...
	protected void populateProductData(final ProductData productData, final Model model)
	{
		...
		model.addAttribute("variantOptionsImages", getImagesForVariantOptions(productData));
	}

	private Map<String, ImageData> getImagesForVariantOptions(final ProductData productData)
	{
		final Map<String, ImageData> variantOptionsImages = new HashMap<String, ImageData>();
		if (productData.getVariantOptions() != null)
		{
			fillVariantOptionsImages(variantOptionsImages, productData.getVariantOptions());
		}
		else if (productData.getBaseProduct() != null)
		{
			for (final BaseOptionData optionData : productData.getBaseOptions())
			{
				fillVariantOptionsImages(variantOptionsImages, optionData.getOptions());
			}
		}
		return variantOptionsImages;
	}

	private void fillVariantOptionsImages(final Map<String, ImageData> variantOptionsImages,
			final List<VariantOptionData> variants)
	{
		for (final VariantOptionData variant : variants)
		{
			for (final VariantOptionQualifierData variantQualifier : variant.getVariantOptionQualifiers())
			{
				if (variantQualifier.getImage() != null)
				{
					variantOptionsImages.put(variantQualifier.getValue(), variantQualifier.getImage());
				}
			}
		}
	}
